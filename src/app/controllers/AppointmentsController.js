...
class AppointmentController {
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      res
        .status(401)
        .json({ erro: 'You can only cancel appointment 2 hous in advance' });
    }
  ...
  async index(req, res) {
    ...
    const appoitments = await Appointment.findAll({
      where: { users_id: req.userId, canceled_at: null },
      order: ['date'],
      attributes: ['id', 'date'],
      limit: 20,
      offset: (page - 1) * 20,
  ...
  async store(req, res) {
    ...
    /**
     * Check if the provider and the user are different
     */

    if (provider_id === req.userId) {
      return res
        .status(400)
        .json({ erro: 'User must be diferent from the provider' });
    }
